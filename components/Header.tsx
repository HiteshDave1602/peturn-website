...
export function Header() {
  ...
  return <header className={`site-header ${scrolled ? "scrolled" : ""}`}>
    ...
    <div className="container nav-wrap">
      <a href="#home" className="logo" aria-label="Peturn home"><Image src="/brand/peturn-logo.png" alt="Peturn — Turn Data Into Growth" width={340} height={138} priority /></a>
      <nav className="desktop-nav" aria-label="Main navigation">{navigation.map(item => <a className={active === item.href.slice(1) ? "active" : ""} key={item.href} href={item.href}>{item.label}</a>)}</nav>
